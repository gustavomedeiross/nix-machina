...
#+begin_src emacs-lisp
  ...
  (use-package magit
    ...
    ;; Make RET open the actual worktree file instead of the index version
...
#+begin_src emacs-lisp
  ...
  (use-package claude-code
    ...
    (general-define-key
     ...
     :keymaps '(normal visual emacs override)
     :prefix "SPC"
     "a c" '(claude-code :which-key "Claude Code start")
     "a s" '(claude-code-send-region :which-key "Claude Code send region")
     "a q" '(claude-code-quit :which-key "Claude Code quit")))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defun gm/nix-rebuild ()
    "Rebuild nix-machina configuration."
    (interactive)
    (let ((default-directory "~/nix-machina/"))
      (compile "./bin/build.sh")))
#+end_src
